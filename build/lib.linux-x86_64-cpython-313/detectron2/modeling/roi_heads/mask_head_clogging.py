...
class ParallelAmodalMaskHeadClogging(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        Parameters
        ----------
        input_shape        : ShapeSpec — ROI feature shape from ROIAlign
        num_classes        : int       — number of foreground classes (1 here)
        conv_dim           : int       — channel width of mask-head convolutions
        fm_lambdas         : tuple     — (lambda_4, lambda_5) feature matching
        unet_base_channels : int       — base channel width of the U-Net
        unet_depth         : int       — encoder/decoder depth of the U-Net
        """
        super().__init__()
    ...
    def _forward_train(
        ...
        # Step A — U-Net reconstruction
        unet_logits = self.unet(Mc_a_soft)                        # (N, 1, H, W)
        ...
        F_amodal_attn  = F * Mr_v_soft                            # (N, C, H, W)
    ...
    def _forward_inference(
        ...
        # Module 1
        Mc_a_logits, _ = self.coarse_amodal_head.forward_with_features(F)
        Mc_a_soft = torch.sigmoid(Mc_a_logits)

        # Module 2
        F_vis_attn  = F * Mc_a_soft
        Mr_v_logits, _ = self.refined_visible_head.forward_with_features(F_vis_attn)
        Mr_v_soft = torch.sigmoid(Mr_v_logits)

        # Module 3 — U-Net + refined amodal
        unet_logits = self.unet(Mc_a_soft)
        unet_soft   = torch.sigmoid(unet_logits)

        F_amodal_input = torch.cat([F * Mr_v_soft, unet_soft], dim=1)
