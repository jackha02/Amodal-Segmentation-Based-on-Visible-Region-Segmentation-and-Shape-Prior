...
class Parallel_Amodal_Visible_ROIHeads(ROIHeads):
    ...
    def _forward_mask(self, features, instances):
        ...
        if self.training:
            ...
            if len(mask_logits) >= 2:
                ...
                storage.put_scalar("mask_rcnn/amodal_weights", torch.mean(weights[0]))
                storage.put_scalar("mask_rcnn/visible_weights", torch.mean(weights[1]))
                losses.update({"loss_amask_a": amodal_mask_rcnn_loss(
                    mask_logits[1][0], proposals, weights=weights[1], mode="amodal", version="a")})
            ...
            if len(mask_logits) >= 3:
                ...
                losses.update({"loss_vmask_g": amodal_mask_rcnn_loss(
                    mask_logits[2][1], proposals, self._cfg.MODEL.ROI_MASK_HEAD.GT_VISIBLE_WEIGHT)})

            dataset_name = self._cfg.DATASETS.TEST[0].split("_")[0]
            if self._cfg.MODEL.ROI_MASK_HEAD.RECON_NET.NAME != "" and not os.path.exists("{}_codebook.npy".format(dataset_name)):
                if not self.SPRef:
                    loss_recon = 0
                    ...
                    losses.update({"loss_recon": loss_recon / len(mask_logits)})
            
            # Note that reclassification network is no longer neded given that there is only one class for our application
            """
            if self._cfg.MODEL.ROI_MASK_HEAD.RECLS_NET.NAME != "":
                x = 1 if len(mask_logits) > 1 else 0
                self.pred_class_logits = []
                if self.recls_mode == "filter":
                    losses.update({"loss_recls": self._cfg.MODEL.ROI_MASK_HEAD.RECLS_NET.LAMBDA * mask_recls_filter_loss
                        (self.recls, get_pred_masks_logits_by_cls(mask_logits[x][1], proposals)[0], mask_features,
                        proposals, box_ths=self.recls_box_ths, mask_ths=self.recls_mask_ths, gt_weight=self.gt_weight,
                        pre_logits=self.pred_class_logits)})
                elif self.recls_mode == "adaptive":
                    losses.update(
            ...
            # Implement the clogging extent Mean Absolute Error loss
        
            # Step 1: Grab the logits from the refined amodal and visible modules 
            x_idx = 1 if len(mask_logits) > 1 else 0 # mask_logits[0], [1] contain the coarse and refined masks, respectively 
            pred_amodal_logits = mask_logits[x_idx][0] 
            pred_visible_logits = mask_logits[x_idx][1] 
            
            mask_side_len = pred_amodal_logits.size(2)
            ...
            cls_agnostic = pred_amodal_logits.size(1) == 1
            idx_offset = 0
            
            # Step 2: Extract, crop, and resize ground truth masks to match logit dimensions
            for instances_per_image in proposals:
                ...
                # Use Detectron2's internal tool that matches the dimensions of ROI to the ground truth data
                # Crop and downsample the ground truth mask to match the dimensions of the predicted mask
                gt_a = instances_per_image.gt_masks.crop_and_resize(
                    instances_per_image.proposal_boxes.tensor, mask_side_len
                ).to(device=pred_amodal_logits.device).float()
                
                gt_v = instances_per_image.gt_visible_masks.crop_and_resize(
                    instances_per_image.proposal_boxes.tensor, mask_side_len
                ).to(device=pred_visible_logits.device).float()
                
                gt_amodal_list.append(gt_a)
                gt_visible_list.append(gt_v)
                
                # Extract the specific class logits 
                if cls_agnostic:
                    p_a = pred_amodal_logits[idx_offset : idx_offset+num_instances, 0]
                    p_v = pred_visible_logits[idx_offset : idx_offset+num_instances, 0]
                else:
                    gt_classes = instances_per_image.gt_classes.to(dtype=torch.int64)
                    indices = torch.arange(num_instances) + idx_offset
                    p_a = pred_amodal_logits[indices, gt_classes]
                    p_v = pred_visible_logits[indices, gt_classes]
                
                pred_a_list.append(p_a)
                pred_v_list.append(p_v)
                idx_offset += num_instances
                
            if len(gt_amodal_list) > 0:

                # Concatenate across the batch
                gt_a_masks = cat(gt_amodal_list, dim=0)
                gt_v_masks = cat(gt_visible_list, dim=0)
                
                p_amodal_logits_cat = cat(pred_a_list, dim=0)
                p_visible_logits_cat = cat(pred_v_list, dim=0)
                
                # Step 3: Calculate Ground Truth Clogging Extent

                eps = 1e-6
                area_occluded_gt = torch.clamp(gt_a_masks - gt_v_masks, min=0.0).sum(dim=(1, 2))
                area_amodal_gt = gt_a_masks.sum(dim=(1, 2))
                ce_gt = area_occluded_gt / (area_amodal_gt + eps) # add a small epsilon to prevent division by 0
                
                # Step 4: Calculate Predicted Clogging Extent
                p_amodal = torch.sigmoid(p_amodal_logits_cat)
                p_visible = torch.sigmoid(p_visible_logits_cat)
                
                p_occluded = torch.clamp(p_amodal - p_visible, min=0.0)
                area_occluded_pred = p_occluded.sum(dim=(1, 2))
                area_amodal_pred = p_amodal.sum(dim=(1, 2))
                ce_pred = area_occluded_pred / (area_amodal_pred + eps)
                
                # Step 5: Compute MAE Loss
                losses["loss_clogging_extent"] = F.l1_loss(ce_pred, ce_gt)
            else:
                losses["loss_clogging_extent"] = pred_amodal_logits.sum() * 0.0   
    ...
    def attention_weights(self, mask_logits, instances):
        ...
        indices = (~torch.isfinite(amodal_weights)).nonzero()
        amodal_weights[indices[:, 0]] = 0
        indices = (~torch.isfinite(visible_weights)).nonzero()
